...
pub fn restore_backup(
    ...
    // The one that would hurt most: somebody restores what they lost, the game
    // saves over it a minute later, and they have spent their backup.
    game_must_be_closed(game)?;
...
/// One character's pins, for a save the launcher can see.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct SlotMarkers {
    pub slot: usize,
    pub name: String,
    pub markers: Vec<crate::markers::Marker>,
}

/// Every marker in a save, character by character.
pub fn read_markers(path: &Path) -> Result<Vec<SlotMarkers>> {
    let save = load(path)?;
    let mut out = Vec::new();
    for slot in 0..crate::formats::save::SLOT_COUNT {
        let Ok(data) = save.slot_data(slot) else {
            continue;
        };
        let markers = crate::markers::read(data);
        if markers.is_empty() && !save.is_slot_active(slot).unwrap_or(false) {
            continue;
        }
        out.push(SlotMarkers {
            slot,
            name: save
                .slot_summary(slot)
...
/// Refuses to touch a save while the game that owns it is running.
///
/// The game holds the whole save in memory and writes it out on its own
/// schedule, so anything written underneath it is thrown away at the next
/// autosave. This used to be a sentence in a doc comment and nothing else,
/// which meant a player who asked the assistant to pin where they were
/// standing — mid-game, the only time anybody would ask — was told it was on
/// their map when it was already gone, and had their save file written under a
/// process that was about to overwrite it.
///
/// The check lives here, at the write, rather than at each of the places that
/// call one: the assistant's tool, the buttons in the interface, and the
/// endpoints behind them. Guarding the callers means guarding them again every
/// time another one is added, and the one that gets forgotten is the one that
/// loses somebody's characters.
///
/// It is skipped when the checks are running. They write to scratch copies of a
/// save, never to one a game could have open, so they are not what this is for
/// — and on a machine with the game running, which is most of them while any of
/// this is being worked on, every one of them would fail on a hazard that is
/// not there. `cfg!(test)` is settled at compile time, so a real build has no
/// way past this at all.
fn game_must_be_closed(game: Game) -> Result<()> {
    if cfg!(test) {
        return Ok(());
    }
    ...
    if crate::launch::is_game_running(game, &system) {
        // "Close it" means quit to the desktop. Told only that the game had to
        // be closed, the assistant offered going back to the main menu, which
        // is not enough — the process is still up and still holds the save, so
        // the player would follow the advice and be refused again.
        return Err(Error::msg(
            "the game is running, and it would overwrite this at its next save — quit the game \
             all the way to the desktop first (going back to the main menu is not enough, the \
             game still holds the save), then this will hold",
        ));
    }
    Ok(())
}

/// Pins one place on a character's map.
///
/// The game must be closed; see `game_must_be_closed`.
pub fn add_marker(
    app_data: &Path,
    game: Game,
    path: &Path,
    ...
    y: f32,
    icon: u8,
) -> Result<crate::markers::Marker> {
    game_must_be_closed(game)?;
    let mut save = load(path)?;
    let backup = create_backup(app_data, game, path, "before adding a marker", true)?;
    let placed = crate::markers::place(save.slot_data_mut(slot)?, x, y, icon)?;
...
/// Takes one pin off, or all of them when given no id.
pub fn remove_markers(
    ...
    game_must_be_closed(game)?;
    let mut save = load(path)?;
    let backup = create_backup(app_data, game, path, "before removing markers", true)?;
    let data = save.slot_data_mut(slot)?;
    let gone = match id {
        Some(id) => usize::from(crate::markers::erase(data, id)),
        None => crate::markers::erase_all(data),
    };
    if gone == 0 {
        return Ok(0);
    }
...
pub fn convert(
    ...
    // Whichever of the two they are playing, one of them is live.
    game_must_be_closed(game)?;
...
mod tests {
    ...
    /// The same, with a map one of the characters can pin.
    fn write_save_with_a_map(path: &Path, steam_id: u64, slot: usize) {
        let mut bytes = vec![0u8; 0x19003B0 + 0x60000];
        bytes[0..4].copy_from_slice(b"BND4");
        let mut save = SaveFile::from_bytes(bytes).unwrap();
        save.set_steam_id(steam_id);
        ...
        let array = crate::markers::blank();
        let data = save.slot_data_mut(slot).unwrap();
        data[0x1000..0x1000 + array.len()].copy_from_slice(&array);
    ...
    #[test]
    fn markers_survive_a_trip_through_the_file() {
        let dir = scratch("markers");
        let app_data = dir.join("appdata");
        let save = dir.join("ER0000.sl2");
        write_save_with_a_map(&save, 7, 1);

        assert!(read_markers(&save).unwrap()[0].markers.is_empty());

        let placed =
            add_marker(&app_data, Game::EldenRing, &save, 1, 4354.01, 8199.27, 0).unwrap();
        assert_eq!(placed.id, 1);

        let found = read_markers(&save).unwrap();
        let mine = found.iter().find(|s| s.slot == 1).expect("slot 1 is there");
        assert_eq!(mine.markers.len(), 1);
        assert!((mine.markers[0].x - 4354.01).abs() < 0.01);

        // The file must still be one the game will load.
        assert!(load(&save).unwrap().verify_checksums());
        // And the old file must still exist, because a write to a save the
        // player cannot replace is the one mistake that cannot be undone.
        assert_eq!(list_backups(&app_data, Game::EldenRing).len(), 1);

        assert_eq!(
            remove_markers(&app_data, Game::EldenRing, &save, 1, Some(placed.id)).unwrap(),
            1
        );
        assert!(read_markers(&save).unwrap()[0].markers.is_empty());
        assert!(load(&save).unwrap().verify_checksums());
    ...
    #[test]
    fn a_slot_with_no_character_refuses_a_marker() {
        let dir = scratch("markers-empty");
        let app_data = dir.join("appdata");
        let save = dir.join("ER0000.sl2");
        write_save(&save, 7, Some(0));

        let refused = add_marker(&app_data, Game::EldenRing, &save, 0, 1.0, 2.0, 0);
        assert!(refused.is_err(), "{refused:?}");
        // And nothing was written over: the save still loads.
        assert!(load(&save).unwrap().verify_checksums());
        assert_eq!(
            remove_markers(&app_data, Game::EldenRing, &save, 0, None).unwrap(),
            0
        );
    }
