...
fn me3_candidates() -> Vec<PathBuf> {
    ...
    // Asked for rather than assumed: Windows is not always on C, and Program
    // Files can be moved. Written out, this found nothing on a machine whose
    // system drive was D.
    for key in ["ProgramFiles", "ProgramFiles(x86)", "ProgramW6432"] {
        let Some(base) = std::env::var_os(key) else {
            continue;
        };
        let me3 = PathBuf::from(base).join("me3");
        candidates.push(me3.join("bin").join("me3.exe"));
        candidates.push(me3.join("me3.exe"));
    }
