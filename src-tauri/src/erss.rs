...
/// FSR's quality modes, out of AMD's own `ffx_upscale.h`.
///
/// `FFX_UPSCALE_QUALITY_MODE_*`, with the ratio each one renders at written in
/// the header beside it: native 1.0x, quality 1.5x, balanced 1.7x, performance
/// 2.0x, ultra performance 3.0x. Confirmed against this installation, which
/// held 0 and then 2 while somebody moved the mod's own slider from native to
/// balanced.
const FSR_MODES: &[(&str, &str)] = &[
    ("0", "Native AA"),
    ("1", "Quality"),
    ("2", "Balanced"),
    ("3", "Performance"),
...
/// XeSS's, out of Intel's own `xess.h`.
///
/// `xess_quality_settings_t` counts from a hundred, which is why the number in
/// the file looks nothing like the others. Confirmed the same way: this
/// installation held 106 and then 101.
///
/// Ultra quality plus is left out on purpose. It is in the enum and the mod may
/// or may not accept it; offering a mode that silently does nothing is worse
/// than not offering it, and there is no way to tell from here which it is.
const XESS_MODES: &[(&str, &str)] = &[
    ("106", "Native AA"),
    ("104", "Ultra quality"),
    ("103", "Quality"),
    ("102", "Balanced"),
    ("101", "Performance"),
    ("100", "Ultra performance"),
];

/// A plain switch. The mod writes these as TOML booleans, unquoted, so the
/// values here are the literals rather than "0" and "1" — the same key written
/// as a number is a key the mod does not read.
const TOGGLE: &[(&str, &str)] = &[("false", "Off"), ("true", "On")];
...
const KNOWN: &[Known] = &[
    Known {
        ...
        choices: UPSCALERS,
        // Said to need a restart because it is NOT established that it does
        // not, and this is the side to be wrong on.
        //
        // It was `false`, which promises the player it takes effect the moment
        // they pick it. Reported from the game: changing it here appeared to do
        // nothing, and changing the same setting in the mod's own panel changed
        // the picture at once. What is actually known:
        //
        //   the write lands correctly — `[Renderer] ScalingMode = "FSR3U"`, a
        //   string, in its section, which is the thing that used to be wrong;
        //   the mod DID switch upscalers around that time, its log showing an
        //   XeSS context destroyed and an FSR one created;
        //   which of the two switches caused that cannot be told apart from
        //   the log, and the panel was in use in the same minute.
        //
        // So the honest reading is that it may or may not be picked up from
        // the file, and telling somebody to restart when they need not is a
        // small annoyance where the reverse is a setting they believe they
        // changed and did not. Settle it by changing this alone, with the mod's
        // panel closed, and watching the log for a context being recreated.
        restart: true,
    },
    Known {
        path: "DLSS.DLSSMode",
    ...
    // The same control the DLSS entry above gives, for the two upscalers a
    // player without an NVIDIA card is left with. Neither was offered, so
    // picking FSR or XeSS meant losing the one setting that decides how the
    // game looks — and the launcher's own reason for existing is not to be
    // written for one machine.
    Known {
        path: "FSR3U.QualityMode",
        title: "FSR quality",
        detail: "How far below your resolution it renders, when the upscaler above is FSR. \
                 Native AA does not scale down at all: no speed, the best picture.",
        choices: FSR_MODES,
        restart: false,
    },
    Known {
        path: "XESS.QualityMode",
        title: "XeSS quality",
        detail: "The same, when the upscaler above is XeSS. Native AA does not scale down.",
        choices: XESS_MODES,
        restart: false,
    },
    Known {
    ...
    // The master switch, which is not the same key as the mode and was not
    // here at all.
    //
    // `FrameGeneration.FrameGenMode` picks WHICH generator; this says whether
    // any of it runs. With this false, choosing DLSS above changes the file and
    // nothing else, and the player has every reason to think the launcher is
    // broken. Right under the mode, because that is where somebody will be
    // looking when it does not work.
    Known {
        path: "FrameGeneration.EnableFrameGen",
        title: "Frame generation on",
        detail: "The switch the mode above needs. Off, nothing is generated no matter which \
                 generator is picked.",
        choices: TOGGLE,
        restart: true,
    },
    ...
    Known {
        path: "DLSS-G.LimitFrameRate",
        title: "Cap counts generated frames",
        detail: "Whether the limit above counts the made-up frames or only the real ones. On, \
                 a cap of 120 with doubling means 60 real; off, it means 120 real and 240 \
                 shown.",
        choices: TOGGLE,
        restart: false,
    },
    Known {
        path: "ShowMetricsWindow",
        title: "The mod's own readout",
        detail: "Frame times and what the generator is doing, drawn over the game by the mod \
                 itself. Useful while setting this up and in the way afterwards.",
        choices: TOGGLE,
        restart: false,
    },
    Known {
...
"#;
    ...
    fn the_mod_can_be_made_to_stop_announcing_itself() {
        ...
        // The readout goes too. It is drawn over the game by the mod itself,
        // in the mod's own style, and leaving it up while hiding the overlay
        // hides nothing — it is the most obvious tell there is.
        assert!(done.contains(&"ShowMetricsWindow".to_string()), "the readout: {done:?}");
        assert!(text.contains("ShowMetricsWindow = false"), "and off, not \"false\": {text}");

        // Settings this build of the mod does not have are skipped, never
        // invented. Named here rather than picked from the real list, because
        // the last key used for this became a real one and the check went on
        // passing while proving nothing.
        assert!(!done.iter().any(|one| one == "NoSuchSettingAnywhere"));
        assert!(!text.contains("NoSuchSettingAnywhere"), "a key was invented: {text}");
    ...
    fn a_value_keeps_the_type_the_mod_gave_it() {
        ...
        // The switches offered as choices go in as booleans, because that is
        // the one way this can go quietly wrong: `TOGGLE` hands the UI the
        // strings "true" and "false", and a switch written to the file with
        // quotes round it is a switch the mod reads as nothing.
        // By content, not by pointer: two identical `&'static` slices are not
        // promised to be the same address, and a filter that quietly matches
        // nothing leaves this whole check passing without running.
        let switches: Vec<&str> =
            KNOWN.iter().filter(|k| k.choices == TOGGLE).map(|k| k.path).collect();
        assert!(!switches.is_empty(), "no switch was checked, so this proved nothing");
        for path in switches {
            for wanted in ["true", "false"] {
                set_setting(&game, path, wanted).unwrap_or_else(|e| panic!("{path}: {e}"));
                let text = std::fs::read_to_string(config_path(&game)).unwrap();
                let key = path.rsplit('.').next().unwrap();
                assert!(
                    text.contains(&format!("{key} = {wanted}")),
                    "{key} was not written as a boolean {wanted}: {text}"
                );
            }
        }
