...
/// one.
/// The memorised spells are **not** in here, and looking again is wasted time.
///
/// The same probe that found the equipment was run over twelve kilobytes of the
/// block, putting every number to the game to be named as a castable good. It
/// found inventory — a blood clot, a hero rune, the flask, a remembrance — and
/// no spells at all. The slots evidently hold a handle into the inventory
/// rather than a spell's own id, which is a different structure and a deeper
/// job than this was.
mod gear {
...
pub struct Gear {
    ...
    /// The ids behind the weapon names, so what they actually do can be looked
    /// up in the installed regulation. Kept alongside rather than instead: the
    /// player knows their weapon by its name, and the number is only useful to
    /// whatever reads the tables.
    pub weapon_ids: Vec<(String, i64)>,
}
...
fn what_they_carry(
    ...
    for slot in 0..gear::WEAPON_SLOTS {
        let Some(value) = id(slot * 4) else { continue };
        let Some(name) = text.name(Kind::Weapon, value) else {
            continue;
        };
        // Both hands can hold the same weapon, and saying it twice reads as a
        // mistake rather than as two of them.
        if carried.weapons.contains(&name) {
            continue;
        }
        carried.weapons.push(name.clone());
        // The whole id, level and all. The tables are keyed on the base — +5
        // Reduvia is row 1040000, not 1040005 — but the level is what turns the
        // table's figures into the ones on the player's screen, so throwing it
        // away here is throwing away the answer.
        carried.weapon_ids.push((name, i64::from(value)));
    }
...
mod tests {
    ...
    /// Where equip load lives, if it lives in this block at all.
    ///
    /// It is the missing half of every armour question. The launcher reads what
    /// each piece weighs and what the player is wearing, so it can say a set
    /// weighs 34 — and cannot say whether they can carry it, which is the only
    /// part they wanted. Today the answer is "that is on your equipment
    /// screen", which is honest and useless.
    ///
    /// Found the way the other offsets here were: by fingerprint rather than by
    /// counting. The weight of what they have on is computable from the tables,
    /// so the float in memory that equals it is the current load, and whatever
    /// sits beside it at a plausible ratio is the maximum. Anything matching by
    /// luck is filtered by asking for the pair.
    ///
    /// **It is not in this block, and that is measured rather than assumed.**
    /// Two kilobytes from `GameDataMan` hold 512 floats and exactly FIVE of
    /// them fall anywhere between 0.1 and 400 — the block is integers and
    /// pointers almost end to end. A load and its maximum would both sit in
    /// that range, so they are elsewhere: the equipment structure hangs off
    /// `WorldChrMan`, which is a different chain and the place to look next.
    ///
    /// One trap already sprung, in case the next attempt reaches for it: the
    /// first cut of this required a fractional part, on the reasoning that a
    /// weight has one. A load of exactly 34.0 does not, and the filter took
    /// hundreds of candidates down to five and found nothing. The probe was
    /// hiding the answer from itself, and the count above is what it looks
    /// like once that is taken off.
    ///
    /// `cargo test --lib show_equip_load -- --ignored --nocapture`
    #[test]
    #[ignore = "a probe, and it needs the game running"]
    fn show_equip_load() {
        let game = Game::EldenRing;
        let Some(live) = read(game) else {
            println!("  the game is not running");
            return;
        };
        let Some(gear) = &live.gear else {
            println!("  nothing equipped, or the gear read failed");
            return;
        };
        println!("\n  {} at level {}", live.name, live.level);
        println!("  stamina {}/{}", live.stamina, live.stamina_max);
        println!("  gear: {gear:?}");

        // The block again, since `read` does not hand it back.
        let Some(pid) = crate::unlock::running_pid(game.executable()) else {
            return;
        };
        let Ok(process) = crate::unlock::win::Process::open(pid) else {
            return;
        };
        ...
        let image = process.read(base, size);
        let Some(found) = crate::unlock::find_only(&image, &crate::unlock::parse(GAME_DATA_MAN))
        else {
            return;
        };
        let Some(bytes) = image.get(found + 3..found + 7) else {
            return;
        };
        let displacement = i32::from_le_bytes(bytes.try_into().unwrap());
        let Some(slot) = (base + found + 7).checked_add_signed(displacement as isize) else {
            return;
        };
        let (Some(manager), ..) = (pointer(&process, slot), ()) else {
            return;
        };
        let Some(data) = pointer(&process, manager + 0x08) else {
            return;
        };
        // Wider than `read` takes, because there is no reason to think this
        // sits inside the part already mapped.
        let block = process.read(data, 0x800);

        // Every float in the block that could be a weight at all, with its
        // offset. A load is tens, not thousands, and never negative.
        let mut plausible: Vec<(usize, f32)> = Vec::new();
        for at in (0..block.len().saturating_sub(4)).step_by(4) {
            let value = f32::from_le_bytes(block[at..at + 4].try_into().unwrap());
            // No filter on being fractional: a load of exactly 34.0 is a load,
            // and requiring a fraction cut this from hundreds of candidates to
            // five and found nothing. That was the probe hiding the answer
            // from itself.
            if value.is_finite() && (0.1..400.0).contains(&value) {
                plausible.push((at, value));
            }
        }
        println!("\n  {} floats in range (v2)\n", plausible.len());

        // Pairs where one is a sensible fraction of the other are what a load
        // and its maximum look like. Printed with the ratio so the pair that is
        // actually them stands out against coincidence.
        for (at, value) in &plausible {
            for (other_at, other) in &plausible {
                if other_at <= at || other <= value {
                    continue;
                }
                let share = value / other;
                if (0.05..0.99).contains(&share) && other_at - at <= 0x40 {
                    println!(
                        "  0x{at:03x} {value:7.2}  /  0x{other_at:03x} {other:7.2}  = {:.0}%",
                        share * 100.0
                    );
                }
            }
