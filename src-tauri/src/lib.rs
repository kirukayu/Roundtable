...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Catch the game's own catalogue of names while it is up, once, and
            // write it down.
            //
            // A plain installation keeps all of that text inside sixty
            // gigabytes of encrypted archive that nothing here opens, so with
            // the game shut such a player had no names at all — no talismans,
            // no upgrade materials, nothing to look an item up by. The same
            // text walks past in memory every time they play, which costs
            // nothing to read and needs no keys.
            //
            // Polled rather than done at startup, because the game is almost
            // never running when the launcher opens. It stops as soon as it has
            // something, and it never runs while the player is waiting on an
            // answer — this is its own thread.
            let catalogue = Arc::clone(&*app.state::<Arc<AppState>>());
            std::thread::spawn(move || {
                for _ in 0..240 {
                    std::thread::sleep(std::time::Duration::from_secs(60));
                    let game = catalogue.settings.lock().selected_game;
                    if crate::text::write_catalogue(&catalogue.app_data, game) > 0 {
                        return;
                    }
                }
            });
