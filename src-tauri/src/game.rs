...
/// whatever their owner felt like, and no list of expected names will ever cover
/// that — so the game is recognised by what is inside a folder, never by what it
/// is called.
...
mod tests {
    ...
    fn a_folder_holding_the_archives_is_recognised_whatever_it_is_called() {
        ...
        let odd = base.join("a folder named nothing like a game").join("g");
        ...
        let found = Installation::probe(Game::EldenRing, &base.join("a folder named nothing like a game"))
            .expect("resolved");
