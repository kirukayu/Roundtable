...
impl SaveFile {
    ...
    /// One slot's payload, borrowed rather than copied.
    pub fn slot_data(&self, index: usize) -> Result<&[u8]> {
        Self::check_slot(index)?;
        let (start, end) = Self::slot_data_range(index);
        Ok(&self.bytes[start..end])
    }

    /// One slot's payload, to be edited in place.
    ///
    /// Whoever writes through this owes the file a `recompute_checksums`; the game
    /// refuses a slot whose MD5 no longer matches.
    pub fn slot_data_mut(&mut self, index: usize) -> Result<&mut [u8]> {
        Self::check_slot(index)?;
        let (start, end) = Self::slot_data_range(index);
