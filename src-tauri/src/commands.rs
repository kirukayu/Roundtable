    ...
    pub fn new(app_data: PathBuf) -> AppState {
        ...
        AppState {
            ...
            http: reqwest::Client::builder()
                ...
                // Deliberately not a total time limit. `timeout` in reqwest
                // covers the body as well as the response, so the flat thirty
                // seconds that used to be here cut off any answer that took
                // longer than that to write — the assistant's answers stream in
                // over one request that lasts as long as the model takes, and
                // the good long ones are exactly the ones that got truncated,
                // mid-word, with nothing to say why.
                //
                // What is worth giving up on is a connection that has gone
                // quiet, so that is what is measured. Calls that do want a hard
                // ceiling set their own per-request `timeout`, and several do.
                //
                // Ninety seconds of quiet, not thirty. Thirty was measured and
                // it was wrong: the assistant's tool rounds go through the
                // service's plain `/chat`, which sends nothing at all until the
                // model has finished writing, so a slow lane on a big round is
                // silent for the whole of it. A question about a spell's cost
                // died three times in a row at exactly thirty seconds each,
                // took a hundred and three seconds to fail, and told the player
                // the service could not be reached. It could. It was answering.
                .connect_timeout(std::time::Duration::from_secs(10))
                .read_timeout(std::time::Duration::from_secs(90))
                // There was a `pool_idle_timeout(5s)` here and it is gone,
                // because it was a guess that was put on trial and lost. It is
                // written down so nobody spends another evening on it.
                //
                // The guess: a round of tool work sits between requests, so a
                // pooled connection can be idle for seconds and be closed at
                // the far end while this side still believes in it. Retiring
                // idle connections early would then cost one handshake and buy
                // back the questions being lost to a 503 page.
                //
                // The verdict: it made no difference and the rate got worse.
                // Battery 42 lost TEN of twelve questions with it in. The two
                // that came back took 8s and 11s and used one tool call; every
                // one of the ten failures ran 64 to 84 seconds. Duration
                // predicts failure, and nothing about connection reuse does.
                //
                // What that battery did establish — and this part is measured,
                // not guessed — is that the answering service is not involved:
                //   * `/health` answered 200 with 122,298 requests left for the
                //     day and all but three of eighty-eight lanes ready.
                //   * A live `wrangler tail` through the failures logged /chat
                //     calls returning 200 OK in 4.4s and 4.8s.
                //   * The worker's own out-of-lanes reply is `json(result, 503)`
                //     — JSON. It cannot produce the HTML page that comes back.
                // So for a failed round no model was ever asked, and the page
                // was made by something between here and Cloudflare.
                .build()
                .unwrap_or_default(),
