...
export type AskEvent =
  ...
  /** `cut` means the answer stops early — say so rather than letting a
      sentence that ends mid-word pass for the whole of it. */
  | { kind: "done"; lane: string | null; ms: number | null; cut?: boolean }
...
/** One pin on the world map, as the save holds it. */
export interface Marker {
  id: number;
  /** Map coordinates, not world ones. */
  x: number;
  y: number;
  /** Which pin the map draws. Zero is the plain one. */
  icon: number;
}
...
/** Somewhere the launcher can pin, named the way the installed game names it. */
...
/**
 * The places, with where they were read from.
 *
 * An empty list has several causes — no game chosen, no mod installed, a
 * language the mod does not carry — and the interface has to say which.
 */
