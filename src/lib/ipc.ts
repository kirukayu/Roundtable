...
export const api = {
  ...
  markersRead: (path: string) => get<SlotMarkers[]>("/markers", { path }),
  markersPlaces: (game: GameId) => get<Places>("/markers/places", { game }),
  markersAdd: (game: GameId, path: string, slot: number, x: number, y: number) =>
    post<Marker>("/markers/add", { game, path, slot, x, y }),
  markersRemove: (game: GameId, path: string, slot: number, id: number | null) =>
    post<number>("/markers/remove", { game, path, slot, id }),
