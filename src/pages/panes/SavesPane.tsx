...
export default function SavesPane({ gameId }: { gameId: GameId }) {
  ...
  const [markers, setMarkers] = useState<Character | null>(null);
  ...
  return (
    <div className="col">
          ...
          <AnimatePresence mode="popLayout">
            {shown.map((character) => (
              <CharacterCard
                ...
                onMarkers={() => setMarkers(character)}
              />
            ))}
          </AnimatePresence>
        </motion.div>
      ...
      {markers && (
        <MarkersModal gameId={gameId} character={markers} onClose={() => setMarkers(null)} />
      )}
  ...
  return (
    ...
    >
      ...
      <footer className="sv__acts">
        ...
        <button type="button" className="btn btn--ghost btn--sm" onClick={onMarkers}>
          Markers
        </button>
        <button
          type="button"
...
/* ── The pins on one character's map ──────────────────────────────── */

/**
 * Markers, which live in the save rather than anywhere the game exposes.
 *
 * Written with the game closed, because it keeps the map in memory while it
 * runs and its next autosave would throw this away. The places come out of the
 * installation itself, so a total conversion's own names and its own map are
 * what you pick from.
 */
function MarkersModal({
  gameId,
  character,
  onClose,
}: {
  ...
  const toast = useToast();
  const [pins, setPins] = useState<Marker[] | null>(null);
  const [places, setPlaces] = useState<Place[] | null>(null);
  const [language, setLanguage] = useState<string>("");
  const [query, setQuery] = useState("");
  const [busy, setBusy] = useState(false);

  const path = character.entry.path;
  const slot = character.slot.index;

  const load = useCallback(async () => {
    const slots = await api.markersRead(path);
    setPins(slots.find((one) => one.slot === slot)?.markers ?? []);
  }, [path, slot]);

  useEffect(() => {
    void load();
    void api
      .markersPlaces(gameId)
      .then((found) => {
  ...
  // Only what the typing narrows to, and not the whole four hundred at once:
  // a list that long is not a list, it is a wall.
  const matches = useMemo(() => {
    const wanted = query.trim().toLowerCase();
    if (!wanted || !places) return [];
    return places.filter((place) => place.name.toLowerCase().includes(wanted)).slice(0, 8);
  }, [places, query]);

  const pin = async (place: Place) => {
    setBusy(true);
    try {
      const done = await toast.run(`Pinned ${place.name}`, () =>
        api.markersAdd(gameId, path, slot, place.mapX, place.mapY),
      );
      if (done) {
        setQuery("");
        await load();
  ...
  const clear = async (id: number | null) => {
    setBusy(true);
    try {
      const done = await toast.run(id === null ? "Map cleared" : "Marker removed", () =>
        api.markersRemove(gameId, path, slot, id),
      );
      if (done) await load();
    } finally {
      setBusy(false);
  ...
  return (
    <Modal
      title={`${character.slot.name.trim() || "This character"}'s map`}
      onClose={onClose}
      footer={
        <>
          <button
    ...
    >
      <div className="col">
        <div className="fld">
          <label className="fld__l" htmlFor="mk-find">Pin a place</label>
          <input
            id="mk-find"
            className="inp"
            placeholder={places === null ? "Reading the game's own place names…" : "Start typing"}
            value={query}
        ...
        {places !== null && places.length === 0 && (
          <div className="note note--warn">
            ...
            <div className="note__b">
              The game's place names could not be read. They come out of the installed files, so
              this needs the game located and, for a total conversion, its own folder known.
            </div>
          </div>
        )}
        ...
        {matches.length > 0 && (
          <div className="rows">
            {matches.map((place) => (
              <div key={`${place.name}-${place.mapX}`} className="rw">
                <span className="rw__t grow truncate">{place.name}</span>
                <button
                  type="button"
                  className="btn btn--ghost btn--sm"
                  disabled={busy}
                  onClick={() => void pin(place)}
                >
                  Pin
                </button>
              </div>
        ...
        <div className="fld">
          ...
          {pins === null ? (
            ...
            <div className="note">
              <Icon.Info size={15} />
              <div className="note__b">Nothing pinned yet.</div>
            </div>
          ) : (
            <div className="rows">
              {pins.map((marker) => (
                <div key={marker.id} className="rw">
                  <span className="rw__t grow">
                    Marker {marker.id}
                    <span className="dim">
                      {" "}
        ...
        <div className="note">
          <Icon.Info size={15} />
          <div className="note__b">
            The game has to be closed: it holds the map in memory while it runs and its next save
            would write over this. The file is snapshotted before every change.
          </div>
        </div>
      </div>
    </Modal>
